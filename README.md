...
> **架构升级 (2026-05-25)**: v2.0 重构为**纯 Skill 驱动架构**，完全可移植到任何 Agent 环境（QClaw、Claude Code、Hermes 等）。
...
1. **纯 Skill 驱动**（非脚本驱动）
   - 改前：`python3 ~/.qclaw/skills/.../scripts/xxx.py`（硬编码路径）
...
2. **可移植性**（相对路径发现机制）
   - 问题：不同 agent 的 skill 安装路径不同（QClaw: `~/.qclaw/skills/`，OpenClaw: `~/.openclaw/skills/` 等）
   - 解决方案：相对路径发现（不绑死特定路径）
   - 查找规则：
...
| 依赖 | 说明 | 安装方式 |
|------|------|---------|
| **QClaw** | Agent 运行环境 | [QClaw 官网](https://qclaw.com) |
...
# 2. 安装 md-wechat-publish-skill 到 QClaw
mkdir -p ~/.qclaw/skills/md-wechat-publish-skill
cp -r md-wechat-publish-skill/* ~/.qclaw/skills/md-wechat-publish-skill/
...
#    或手动克隆：
#    git clone https://github.com/stefanxfy/md-mermaid2img-skill.git ~/.qclaw/skills/md-mermaid2img-skill
...
```bash
# 检查 QClaw skill 目录
ls ~/.qclaw/skills/md-wechat-publish-skill/
ls ~/.qclaw/skills/md-mermaid2img-skill/
ls ~/.qclaw/skills/jimeng-image-gen/
...
- ✅ **相对路径发现**，不绑死特定 agent 路径（如 `~/.qclaw/skills/`）
...
| QClaw | [qclaw.com](https://qclaw.com) |
...
- **可移植性**：相对路径发现机制，不绑死特定 agent 路径
  ...
  - 支持 QClaw、Claude Code、Hermes 等任何有 skill 系统的 agent 环境
